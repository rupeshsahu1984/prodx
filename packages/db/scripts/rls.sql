...
/**
 * Installed industry packs (ADR 0009). Same shape as the plant scope: NULL only
 * via the explicit '*' sentinel, empty otherwise, so a pack table is invisible
 * unless the caller has said the pack is enabled.
 */
CREATE OR REPLACE FUNCTION app_packs() RETURNS text[] AS $$
  SELECT CASE
    WHEN current_setting('app.packs', true) = '*' THEN NULL
    WHEN coalesce(current_setting('app.packs', true), '') = '' THEN ARRAY[]::text[]
    ELSE string_to_array(current_setting('app.packs', true), ',')
  END
$$ LANGUAGE sql STABLE;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pack_table) THEN
    RAISE EXCEPTION
      'pack_table is empty. Run sync-packs before rls.sql, or pack tables would be left ungated.';
  END IF;
END
$$;
...
BEGIN
  ...
  LOOP
    ...
    -- disabling a pack hides its data everywhere at once — API, reports and
    -- background jobs — with no application check to forget.
    IF owning_pack IS NOT NULL THEN
      predicate := predicate || format(
        ' AND (app_packs() IS NULL OR %L = ANY(app_packs()))', owning_pack);
    END IF;
